...
setup_postgres_schema() {
    ...
    if [[ ${VISIBILITY_DBNAME} != "${VISIBILITY_POSTGRES_USER}" && ${SKIP_DB_CREATE} != true ]]; then
        temporal-sql-tool --plugin postgres --ep "${VISIBILITY_POSTGRES_SEEDS}" -u "${VISIBILITY_POSTGRES_USER}" -p "${VISIBILITY_DB_PORT}" --db "${VISIBILITY_DBNAME}" --pw "${VISIBILITY_POSTGRES_PWD}" create
    fi
    temporal-sql-tool --plugin postgres --ep "${VISIBILITY_POSTGRES_SEEDS}" -u "${VISIBILITY_POSTGRES_USER}" -p "${VISIBILITY_DB_PORT}" --db "${VISIBILITY_DBNAME}" --pw "${VISIBILITY_POSTGRES_PWD}" setup-schema -v 0.0
    temporal-sql-tool --plugin postgres --ep "${VISIBILITY_POSTGRES_SEEDS}" -u "${VISIBILITY_POSTGRES_USER}" -p "${VISIBILITY_DB_PORT}" --db "${VISIBILITY_DBNAME}" --pw "${VISIBILITY_POSTGRES_PWD}" update-schema -d "${VISIBILITY_SCHEMA_DIR}"
}
